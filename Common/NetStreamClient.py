...
class NetStreamClient(object):
	def __init__(self):
		super(NetStreamClient, self).__init__()
	...
	def close(self):
		...
		try:
			...
			logging.error(str(e))                # should logging here
		self.sock = None
		return 0
	...
	def __tryConnect(self):
		...
		try:
			self.sock.recv(0)
		except socket.error as (code, strerror):
